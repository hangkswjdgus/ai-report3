...
7. 수치를 쓸 때는 반드시 다음을 포함할 것:
   - 연도
   - 기관 또는 연구 출처
   - 구체적인 결과
8. 각 문단마다 최소 1개 이상의 구체적 근거(수치 또는 기관명)를 포함할 것.
9. 동일한 문장에서 "연구에 따르면" 같은 표현을 반복하지 말 것.
10. {length_rule}
11. 문체는 {tone}로 작성할 것.
12. 문서 구조를 제목과 소제목으로 명확히 나눌 것.
13. 마지막 참고자료는 실제 사용한 자료만 정리할 것.
14. 반드시 JSON 형식으로만 답할 것.
15. 설명문, 코드블록, 안내문 없이 JSON 객체만 출력할 것.
16. body에는 문서 본문만 넣고, references에는 실제 사용한 참고자료만 넣을 것.
...
- 최소 1개 이상의 숫자 포함 근거를 반드시 포함할 것.
- 수치를 찾지 못한 경우에는 기관명 또는 연구명 기반의 서술로 작성하되, 지어내지 말 것.
"""
